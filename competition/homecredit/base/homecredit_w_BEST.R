...
# mean
submit$TARGET <- output$pred
fwrite(submit, "~/submit_best3.csv")

# rank mean 
cvPreds = do.call(cbind, output$sub_preds)
cvPreds_w_rank = apply(cvPreds, 2, function(x) rank(x)/length(x))
submit$TARGET <- rowMeans(cvPreds_w_rank)
fwrite(submit, "~/submit_best5.csv")
