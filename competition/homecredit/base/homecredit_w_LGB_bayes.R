...
start = Sys.time()
reticulate::source_python("../homecredit/base/LGB/bayesOptimization.py")
best_params1 = bayesOptimization(X=data_x, y=data_y, init_round=init_points, opt_round=n_iter, num_iterations=iterations)
best_params1$History <- data.frame(
  Round = 1:length(best_params1$History),
  rbindlist(best_params1$History$all$params),
  Values = unlist(best_params1$History$all$values)
)
Sys.time() - start
